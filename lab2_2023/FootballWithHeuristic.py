...
class Football(Problem):
    ...
    def h(self, node):
        ...
        list_of_actions = ["UP", "DOWN", "RIGHT", "UP_RIGHT", "DOWN_RIGHT"]
        list_of_moves = []
        for action in list_of_actions:
            next_action = move_player(action, ball_position)
            if is_player_move_valid(next_action, self.opponents):
                list_of_moves.append(next_action)

        list_of_distances = []

        for action in list_of_moves:
            list_of_distances.append(max(manhatten_distance(action, self.goal[0]), manhatten_distance(action, self.goal[1])))
