...
if __name__ == '__main__':
    ...
    type = input()
    new_x = [[int(element) for element in input().split(" ")]]

    for row in fish:
        del row[col_index]
    del new_x[0][col_index]

    x = [row[:-1] for row in fish]
    y = [row[-1] for row in fish]
    ...
    new_y = classifier.predict(new_x)[0]
    print(new_y)
    print(classifier.predict_proba(new_x)[0])
