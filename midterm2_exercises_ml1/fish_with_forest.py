...
def get_data_with_removed_index(index):
    data = get_dataset()
    for data_entry in data:
        del data_entry[index]
...
if __name__ == '__main__':
    ...
    criterion = input()
    x_new = [[float(element) for element in input().split(" ")]]
    del x_new[0][index]

    X, y = get_data_with_removed_index(index)
    x_train, x_test, y_train, y_test = train_test_split(X, y, train_size=0.85, shuffle=False)

    classifier = RandomForestClassifier(criterion=criterion, n_estimators=number_of_trees, random_state=0)
    ...
    class_probs = classifier.predict_proba(x_new)[0]
